...
class Swoole
{
    ...
    static $default_controller = array('module'=>'jome','controller' => 'index', 'action' => 'index');
...
//            set_error_handler('swoole_error_handler');
    ...
    function runMVC()
    {
        ...
        //check module name
        if (!preg_match('/^[a-z0-9_]+$/i', $mvc['module']))
        {
            return Swoole\Error::info('MVC Error!',"controller[{$mvc['module']}] name incorrect.Regx: /^[a-z0-9_]+$/i");
        }
        ...
        //check view name
        if (!preg_match('/^[a-z0-9_]+$/i', $mvc['action']))
        {
        	return Swoole\Error::info('MVC Error!',"view[{$mvc['action']}] name incorrect.Regx: /^[a-z0-9_]+$/i");
        }
        //check app name
        ...
        //使用命名空间，文件名必须大写
        $controller_class = '\\App\\Controller\\'.ucwords($mvc['module']).'\\'.ucwords($mvc['controller']);
        if (self::$controller_path)
        {
            $controller_path = self::$controller_path . '/' . ucwords($mvc['module'])  .'/'. ucwords($mvc['controller'])  . '.php';
        }
        else
        {
            $controller_path = self::$app_path . '/controllers/' . ucwords($mvc['module'])  .'/'. ucwords($mvc['controller']) . '.php';
        }
        ...
        else
        {
            ...
            {
                if(!empty($this->config['apps']['function'])){
                    require_once self::$app_path. '/' . $this->config['apps']['function'];
                }
        ...
        //服务器模式下，尝试重载入代码
        if (defined('SWOOLE_SERVER') && extension_loaded('runkit') && $this->config['apps']['auto_reload'])
        {
            $this->reloadController($mvc, $controller_path);
            $this->reloadPublicFunc(self::$app_path. '/'.$this->config['apps']['function']);

        }

        $controller = new $controller_class($this);
        if (!method_exists($controller, $mvc['action']))
        {
            $this->http->status(404);
            return Swoole\Error::info('MVC Error!'.$mvc['action'],"View <b>{$mvc['controller']}->{$mvc['action']}</b> Not Found!");
        }
        ...
        mylog('return:'.json_encode($return));
        //响应请求
    ...
    function reloadPublicFunc($function_file){
        clearstatcache();
        $res = runkit_import( $function_file,RUNKIT_IMPORT_CLASS_METHODS|RUNKIT_IMPORT_OVERRIDE);
    }
...
function swoole_urlrouter_mvc(&$uri)
{
    ...
    if(!empty(Swoole::$php->config['rewrite'][0]['mvc'])){
        $array = array_merge($array, Swoole::$php->config['rewrite'][0]['mvc']);
    }
    ...
    if(!isset($request[2])){
        ...
        if(!empty($request[0])){
            $array['module'] = $request[0];
        };
        if(!empty($request[1])){
            $array['controller'] = $request[1];
        };
        if(!empty($request[2])){
            $array['action'] = $request[2];
        };
    }
