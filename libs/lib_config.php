...
/*
 * mongodb传入一个collection名,返回一个mongoClient
 * @param $db_name
 * $param $collection_name
 * @return MongoClient
 */
function collection($collection_name, $db_key = 'master'){
    static $db = null;
    if(empty($db)){
        $db_name = Swoole::getInstance()->config['mongo'][$db_key]['name'];
        $db = Swoole::getInstance()->mongo->selectCollection($db_name, $collection_name);
    }
